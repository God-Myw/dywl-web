<template>
  <div
    class="shiBuy"
    v-if="dataList.shipSellerDto"
    :class="{ intervoyageinland: clientSide }"
  >
    <div class="shiBuy-swipe">
      <van-swipe @change="onChange" height="280">
        <van-swipe-item
          v-for="(item, index) in dataList.shipPhotos"
          :key="index"
        >
          <img
            :src="'http://58.33.34.10:10443/images/shiptrade/' + item.fileName"
            style="object-fit: cover"
            alt=""
        /></van-swipe-item>
        <template #indicator>
          <div class="custom-indicator">
            {{ current + 1 }}/{{ dataList.shipPhotos.length }}
          </div>
        </template>
      </van-swipe>
    </div>
    <div class="shiBuy-title">
      <div class="title-top">
        ...
        <div class="title-top-right">
          ...
          <div
            class="price_interview"
            v-if="Number(dataList.shipSellerDto.price)"
          >
      ...
      <div class="title-bottom">
        <div>挂牌编号：{{ dataList.shipSellerDto.orderNumber }}</div>
        <div>{{ dataList.shipSellerDto.views }} 浏览</div>
      </div>
    </div>
    <!-- 船舶档案 -->
    <div class="shiBuy-record">
      <div class="record-title">船舶档案</div>
      <div class="record-nav">
        <div
          class="record-item"
          v-if="dataList.shipSellerDto.classificationSociety"
        >
          <div>船级社</div>
        ...
        <div
          class="record-item"
          v-if="dataList.shipSellerDto.specialInspectionDate"
        >
          <div>特检</div>
          <div>
        ...
        <div
          class="record-item"
          v-if="dataList.shipSellerDto.buildParticularYear"
        >
          <div>建造年份</div>
...
export default {
  ...
  mounted() {
    ...
    this.getweChatPay(
      ...
      this.getQueryVariable("sellerGuid")
    );
  },
  created() {
  ...
  methods: {
        ...
        if (res.code == "0000") {
          ...
          wx.ready(function () {
            var s_title = "道裕物流—船舶挂牌出售", // 分享标题
              s_link =
                "https://www.dylnet.cn/h5share/shiBuy?token=" +
                token +
                "&sellerGuid=" +
                sellerGuid, // 分享链接
    ...
    async getList() {
      let res = await axios.get(
        "https://www.dylnet.cn/api/business/shipSelling/getShipSellingDetailForApp",
        {
          headers: {
