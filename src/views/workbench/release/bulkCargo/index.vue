<template>
	<div class="bulkCargo">
		...
		<div class="header" v-else>
			我有散杂货要运-<span v-if="InternationalTransport == 1">国际</span><span v-else>国内</span>
		</div>
		<div class="cont">
			<!-- 步骤条 -->
			<div class="cont-step">
				...
				<div class="stage-one" v-if="defaultCurrent == 0">
					<div class="ele-sele">
						<div class="ele-sele-beg">
							<div><span style="color: red; padding-right: 2px">*</span> 起运港</div>
							<el-select
						...
						<div class="ele-sele-beg">
							<div><span style="color: red; padding-right: 2px">*</span> 目的港</div>
							<el-select
            ...
            <t-form-item label="目的港" name="college">
              ...
              >
				...
				<!-- 阶段二---表单--国际 -->
				<div class="stage-inter" v-if="defaultCurrent == 1 && InternationalTransport == 1">
							...
							>
								...
								<t-form-item label="是否可叠加" name="isSuperposition">
									<t-radio-group v-model="formData.isSuperposition">
										<t-radio @click.native.prevent="handleRadioClick('0')" value="0">可以</t-radio>
										<t-radio @click.native.prevent="handleRadioClick('1')" value="1"
											>不可以</t-radio
										>
										<t-radio @click.native.prevent="handleRadioClick('2')" value="2"
											>部分可以</t-radio
										>
									</t-radio-group>
								</t-form-item>
								<t-form-item label="是否重大件" name="majorParts">
									<t-radio-group v-model="formData.majorParts">
										<t-radio @click.native.prevent="handleRadioClickmajor('1')" value="1"
											>是</t-radio
										>
										<t-radio @click.native.prevent="handleRadioClickmajor('0')" value="0"
											>否</t-radio
										>
									</t-radio-group>
								</t-form-item>
							...
							<div class="handling-clause">
								...
								<div>
									<t-radio-group v-model="formData.intentClause" allowUncheck>
										<t-radio @click.native.prevent="handleRadioClickClause('FILO')" value="FILO"
											>FILO</t-radio
										>
										<t-radio @click.native.prevent="handleRadioClickClause('FLT')" value="FLT"
											>FLT</t-radio
										>
										<t-radio @click.native.prevent="handleRadioClickClause('LIFO')" value="LIFO"
											>LIFO</t-radio
										>
										<t-radio @click.native.prevent="handleRadioClickClause('FLO')" value="FLO"
											>FLO</t-radio
										>
										<t-radio @click.native.prevent="handleRadioClickClause('FLOST')" value="FLOST"
											>FLOST</t-radio
										>
									</t-radio-group>
								</div>
					...
					<div class="cont-line"></div>
					<div class="cont-nextstep">
						<t-button style="margin-right: 20px" variant="outline" theme="default" @click="lastStep"
							>上一步</t-button
						>
				...
				<!-- 阶段二---表单--国内 -->
				<div class="stage-two" v-if="defaultCurrent == 1 && InternationalTransport != 1">
						...
						<div class="transport-from">
							<div>
								...
								<div class="transport-rate">
									...
									<div>
										<t-radio @change="onChangeRate(1)" :checked="rateValue" :allowUncheck="true"
											>按每吨收费</t-radio
										>
									</div>
									<div>
								...
								<div class="transport-bag">
									<div>
										<t-radio @change="onChangeBag(2)" :checked="bagValue" :allowUncheck="true"
											>按总包干价</t-radio
										>
									</div>
									<div>
			...
			<div class="stage-end" v-if="defaultCurrent == 2">
				...
				<div class="end-btn">
					<t-button
						...
						>返回发布首页</t-button
					>
					<t-button theme="default" @click="gobulk()" variant="outline">继续发布货盘</t-button>
				</div>
			</div>
		</div>
	</div>
	...
	export default {
		...
		data() {
			return {
				...
				receiAddress: [],
				rules: {
					goodsLevel: [{ required: true, message: "请选择货物名称", type: "error" }],
					goodsWeight: [{ required: true, message: "请填写货物最小最大重量", type: "error" }],
					weightMin: [
						{
							required: false,
					...
					goodsVolume: [{ required: false, message: "请填写货物体积", type: "error" }],
					contactPhone: [{ required: true, message: "请填写联系电话", type: "error" }],
					location: [{ required: false, message: "请选择存放位置", type: "error" }],
					goodsProperty: [{ required: false, message: "请选择货物性质", type: "error" }],
					isSuperposition: [{ required: false, message: "请选择是否叠加", type: "error" }],
					majorParts: [{ required: false, message: "请选择是否叠重大件", type: "error" }],
		...
		methods: {
			...
			async goodsProperty() {
				let res = await axios.get("https://www.dylnet.cn/api/sys/dict/type?type=goods_property");
				if (res.data.code == "0000") {
			...
			async onSubmit({ validateResult }) {
				if (this.formData.startPort === "" || this.formData.destinationPort === "") {
			...
			// 阶段一 --- 下一步
			nextStep() {
				if (Number(this.formData.goodsWeight) > Number(this.formData.goodsMaxWeight)) {
			...
			interSubmit({ validateResult }) {
				if (
					new Date(this.formData.loadDate).getTime() > new Date(this.formData.endDate).getTime()
				) {
